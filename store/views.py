...
#view below routes the product to product_detail.html
def product_detail(request, category_slug, product_slug):
    try:
        single_product=Product.objects.get(category__slug=category_slug, slug=product_slug)
        in_cart = CartItem.objects.filter(cart__cart_id=_cart_id(request), # so this is complicated.  looks like foreign key back to cart on left is served by private key function with session id on the right
